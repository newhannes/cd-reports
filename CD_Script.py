...
def create_html_report(congressional_district, national_df, df2020, social_vars_v, econ_vars_v, industry_vars_v, housing_vars_v):
    ...
    for var in df.keys():
        ...
        rank = ranked_df[ranked_df['NAME'] == district].index[0] + 1 # Find the rank of the specified district    
        ...
        state_rank_val = state_ranked_df[state_ranked_df['NAME'] == district].index[0] + 1 # Find the rank of the specified district
...
district = "NC-11"
html_report = create_html_report(district, df, df_2020, social_vars_v, econ_vars_v, industry_vars_v, housing_vars_v)
# with open(f"output/{district} report -Immigrant.html", "w") as f:
#     f.write(html_report)
